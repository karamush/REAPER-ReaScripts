--[[
 * ReaScript Name: Prepare Chords and Lyrics to dedicated web interface (use reaper-ws-proxy app)
 ...
 * Link: https://github.com/karamush/reaper-ws-proxy
 * Licence: GPL v3
 * About:
   Разовый запуск скрипта формирует данные для reaper-ws-proxy (https://github.com/karamush/reaper-ws-proxy) и сохраняет их в ExtState. Данные - это JSON с текстом и позициями из треков Chords и Lyrics. Используйте программу reaper-ws-proxy для показа текстов и аккордов в веб-интерфейсе со скроллингом. Если дорожки или элементы в них поменялись, скрипт нужно запустить заново. Основано на наработках X-Raym, но оптимизировано и адаптировано для передачи через WebSocket-ы. Веб-интерфейс для отображения данных - это отдельный проект, который можно найти по ссылке в описании и доработать под свои нужды.
 * Version: 1.0
--]]
